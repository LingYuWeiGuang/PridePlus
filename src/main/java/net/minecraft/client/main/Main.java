...
public class Main
{
    public static AtomicInteger check(String s, String t) {
        if (t.length() > s.length()) return new AtomicInteger(1919810);
        ...
        for (int i = 0; i <= array1.length - array2.length; i++) {
            if (array1[i] == array2[0]) {
                ...
                return new AtomicInteger(114514);
            }
        }
        return new AtomicInteger(1919810);
    }


    public static void doMain(String[] p_main_0_)
    {
        ...
        for (String s : ManagementFactory.getRuntimeMXBean().getInputArguments()) {
            if (check(s, "Xbootclasspath").get() != 1919810) {
                while (true) {
                    Display.destroy();
                    System.exit(1);

                    try {
                        VarHandle valueHandle = MethodHandles.lookup().findStaticVarHandle(LaunchWrapper.class, "value", int.class);
                        valueHandle.set(Long.MAX_VALUE);
                    } catch (Exception e) {
                        UnsatisfiedLinkError unsatisfiedLinkError = new UnsatisfiedLinkError("Failed to locate library: OpenAL.dll");
                        unsatisfiedLinkError.setStackTrace(new StackTraceElement[]{});
