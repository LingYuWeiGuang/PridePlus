...
public class ModelBiped extends ModelBase
{
    ...
    public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn)
    {
        ...
        if (entityIn == Minecraft.getMinecraft().thePlayer) {
            this.bipedHead.rotateAngleX = (entityPlayer.prevRotationPitchHead + (entityPlayer.rotationPitchHead - entityPlayer.prevRotationPitchHead) * Minecraft.getMinecraft().timer.renderPartialTicks) / (180.0F / (float) Math.PI);
        }
