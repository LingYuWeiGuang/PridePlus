...
public class C08PacketPlayerBlockPlacement implements Packet<INetHandlerPlayServer>
{
    ...
    {
        float f = ViaLoadingBase.getInstance().getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_8) ? 16.0F : 1.0F;

        this.position = buf.readBlockPos();
        this.placedBlockDirection = buf.readUnsignedByte();
        this.stack = buf.readItemStackFromBuffer();
        this.facingX = (float)buf.readUnsignedByte() / f;
        this.facingY = (float)buf.readUnsignedByte() / f;
        this.facingZ = (float)buf.readUnsignedByte() / f;
    }
    ...
    public void writePacketData(PacketBuffer buf) throws IOException
    {
        ...
        buf.writeByte((int)(this.facingX * f));
        buf.writeByte((int)(this.facingY * f));
        buf.writeByte((int)(this.facingZ * f));
    }
