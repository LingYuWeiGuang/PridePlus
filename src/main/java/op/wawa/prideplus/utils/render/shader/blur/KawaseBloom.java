...
public class KawaseBloom implements MinecraftInstance {
    private static final ShaderUtils kawaseDown = new ShaderUtils("kawaseDownBloom");
    private static final ShaderUtils kawaseUp = new ShaderUtils("kawaseUpBloom");

    private static Framebuffer framebuffer = new Framebuffer(mc.getFramebuffer().framebufferWidth, mc.getFramebuffer().framebufferHeight, true);
    ...
    public static void renderBlur(int framebufferTexture, int iterations, int offset, float alpha) {
        ...
        RenderUtils.bindTexture(framebufferList.getFirst().framebufferTexture);
        RenderUtils.setAlphaLimit(0);
        GLUtils.startBlend();
        ShaderUtils.drawQuads();
        GlStateManager.bindTexture(0);
